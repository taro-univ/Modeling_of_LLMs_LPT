#!/bin/bash
# run_full_sweep.sh — フルスイープ（相図 + SG解析 + モデルスイープ）
#
# run_phase_diagram.sh と run_pq_sweep.sh を統合し、1回の実行で以下すべてに
# 対応したデータを収集する:
#   - 2相相図: accuracy (秩序相 vs 崩壊相)
#   - 3相相図: early_stop 内訳 (ordered / spin_glass / paramagnetic)
...
#
# Usage:
#   docker compose exec hanoi-minimal bash runners/scripts/run_full_sweep.sh [OPTIONS]
#
# OPTIONS:
...
#   --ts "0.2 0.4 ..."      温度グリッド（スペース区切り・引用符必須）
...
for MODEL_ID in "${MODELS[@]}"; do
    ...
    if [[ "$DO_ANALYZE" == true && "$DRY_RUN" == false ]]; then
        ...
        FIG_DIR="figures/full_sweep/${SLUG}"
        mkdir -p "$FIG_DIR"
