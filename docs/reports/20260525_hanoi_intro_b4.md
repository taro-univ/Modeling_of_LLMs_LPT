---
title: "LLMの推論行き詰まりを統計力学で測る"
subtitle: "Tower of Hanoi 実験による相転移の観測"
author: "（発表者名）"
date: "2026-05-25"
---
...
なぜ重要か？→ LLMが「賢い」なら制御できるはず。行き詰まりを**検出・回避**できれば推論の質が上がる。
...
- **隠れ状態** $\mathbf{h}$：各 Move 直後のモデル内部ベクトル（$d \sim 4096$ 次元）
...
$q_{ab} \in [-1, 1]$：コサイン類似度（1 = 完全一致、0 = 直交）
...
SG 相では「時に似ている、時に全く違う」という複雑な構造が現れる。
...
![図1: DeepSeek-R1-Distill-Qwen-7B の精度相図（layer_mid）。横軸が生成温度 $T$、縦軸がディスク数 $N$。色が明るいほど正解率が高い（秩序相）。](20260525_hanoi_intro_b4_figures/phase_diagram.png)

**読み方：**
- **左上（低 $T$、小 $N$）**：正解率が高い = 秩序相
- **右側（高 $T$）**：正解率が低い
- **下側（大 $N$）**：温度が低くても正解できなくなる
- **境界線**：相転移点 $T_c(N)$（$N$ が増えると低温側にシフト）

「$N$ が増えると同じ $T$ でも崩壊が速くなる」→ 難しい問題ほど行き詰まりやすい

---

## 実験結果 ─ P(q) 分布の代表例

![図2: 各 $(T, N)$ セルの $P(q)$ 分布グリッド（layer_mid）。横軸が温度 $T$、縦軸がディスク数 $N$。各セルの分布形状が相を反映している。](20260525_hanoi_intro_b4_figures/pq_grid.png)
...
- Hopfield 模型の**容量限界**（$\alpha_c \approx 0.138N_p$）に対応
...
SG シグナル（ループ率）は $T = 0.7$〜$1.0$ でピーク、$T \geq 1.2$ で消滅 = PM 相に飲み込まれる。
...
![図3: 層別（layer_top / layer_mid / layer_low）の $q_{EA}$ と精度 $m$ の関係。各点が1つの $(T, N)$ セル。$q_{EA}$ が大きいほど精度が高い傾向が、温度 $T$ を制御した後も残る。](20260525_hanoi_intro_b4_figures/layer_comparison.png)

**重要な発見：**
- $q_{EA}$ と精度 $m$ には正の相関がある
- $T$ を統制変数として偏相関を取っても相関が残る
- = **$q_{EA}$ は $T$ とは独立した精度の予測変数**

**物理的解釈：** 隠れ状態の「試行間一貫性」は温度だけで決まらない。
問題の複雑度 $N$ や、モデルの内部構造（どの層を見るか）も影響する。
...
1. $(T, N)$ 平面に Ordered / SG / PM の **3 相構造**が存在する
2. $N=4$–$6$ の SG セルで **P(q) 双峰性**が観測された（H3 supported）
3. $q_{EA}$ は $T$ を制御した後も精度の**独立予測変数**として機能する

**現在検証中 / わからないこと：**

1. **相境界のスケーリング則** $T_c(N) = A \cdot N^{-\alpha_s}$ の指数 $\alpha_s$ は未確定（試行数不足）
2. 残り 3 モデル（Llama 8B, Qwen3 7B/14B）の相図 = **普遍性の検証が未着手**
3. **SG 判定基準の定量化**（P(q) の双峰性をどう数値化するか = 現在再設計中）
4. **制御外力**と実推論の対応付け（H4 段階 2, 未検証）

これらは `open_questions.md` の U2〜U5 に記録済み。
...
**今の位置：** 段階 1 完了（7B モデルのみ）、段階 2 への移行準備中
...
**1. LLMの推論行き詰まりを統計力学の相転移として記述できる**

- move loop → SG 相（局所安定状態への収束）
- no move → PM 相（完全無秩序）
- goal reached → Ordered 相（正解到達）

**2. Tower of Hanoi で $(T, N)$ 相図を描くと 3 相構造が現れる**

- $N$ を増やす / $T$ を上げると Ordered → SG → PM へ遷移
- 7B モデルで 86 セルの相図を取得済み

**3. P(q) 双峰性でスピングラス相を同定し、制御モデルへつなぐ**

- H3（move loop = 局所安定状態）が P(q) 解析で支持された
- $q_{EA}$ は精度の独立予測変数 → 制御指標として使える可能性
...
現行の分類アルゴリズムは physics-agent 審査で**不合格（要再設計）**と判定済み。P(q) ベース再設計を進行中。
...
```
ラベルベース（現行）     →     P(q) ベース（新方針）
  early_stop 分類              双峰性指標（BC, dip検定）
  （主観的・行動的）            （客観的・内部状態ベース）
                                    ↓
                              SG 相の同定
                              $q_{EA}$ の計算
...
これにより、ラベルに頼らない**物理的に根拠のある相分類**を目指す。
