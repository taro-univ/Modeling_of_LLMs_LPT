...
```
Phase 1（進行中）: Tower of Hanoi × 4 モデル → P(q) 相図 → H3 支持の確認
                    ↓
...
| 優先度 | 論文 | 理由 |
|---|---|---|
| ⭐⭐⭐ 最優先 | Amit, Gutfreund, Sompolinsky 1987 (AGS), Ann. Phys. 173, DOI: 10.1016/0003-4916(87)90092-3 | 相図の理論的基盤。SPEC-2026-05-22-001 の参照文献 |
...
- [ ] Tower of Hanoi × 4 モデル の full_sweep 完了（Qwen3-8B 進行中・Qwen3-14B 未着手）
- [ ] 4 モデル全ての P(q) 相図生成
- [ ] 相図の比較（モデル間での AGS 構造の一致・差異を確認）
- [x] Lights Out 実装・sweep スクリプト整備完了（2026-05-26）
- [ ] Frog Jump の実装・テスト完了
- [ ] River Crossing の実装・テスト完了
- [ ] AGS 1987 読了（Amit, Gutfreund, Sompolinsky, Ann. Phys. 173, 1987）
- [ ] arxiv:2503.23084 読了
...
- **4 モデル戦略に絞る**（user, 2026-05-26）→ DeepSeek-7B/14B + Qwen3-8B/14B。llama-8B は補足扱い。理由：llama-8B は N≥3 でほぼ acc=0 のため $T_c$ フィッティングに使えない。パズル軸の追加（Lights Out）の方が普遍性主張として価値が高い
- **追加モデルは不要**（user, 2026-05-26）→ 5 モデル目を加えるより 2×2 設計で深く掘る方針
